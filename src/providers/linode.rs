...
impl<'a> ApiCacheFetcher<'a, i64> for LinodeZoneFetcher<'a> {
...
impl<'a> ApiCacheFetcher<'a, i64> for LinodeRecordFetcher<'a> {
    async fn fetch_api_response(&'a self) -> crate::Result<i64> {
        /*  curl -sS --request GET \
...
impl LinodeProvider {
    ...
    pub(crate) async fn create(
        ...
        let zone_id = self
            .zone_cache
            .get_or_update(&LinodeZoneFetcher {
                client: &self.client,
                endpoint: &self.endpoint,
                zone: &zone,
            })
    ...
    pub(crate) async fn update(
        ...
        let record_id = self
            .record_cache
            .get_or_update(&LinodeRecordFetcher {
                zone_id,
                client: &self.client,
                endpoint: &self.endpoint,
                delegate: &delegate,
